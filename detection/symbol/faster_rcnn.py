...
def residual_unit(data, num_filter, stride,\
                  dim_match, name, bottle_neck=True,\
                  bn_mom=0.9, workspace=512, bn_global=True):
    ...
    """
    if bottle_neck:
        # the same as https://github.com/facebook/fb.resnet
        # .torch#notes, a bit difference with origin paper
...
def rpn(data, num_class=2, num_anchor=12):
    ...
    rpn_cls_score = mx.symbol.Convolution(data=rpn_relu, kernel=(1, 1), pad=(0, 0), num_filter=2 * num_anchor,
                                          name="rpn_cls_score")
    rpn_bbox_pred = mx.symbol.Convolution(data=rpn_relu, kernel=(1, 1), pad=(0, 0), num_filter=4 * num_anchor,
                                          name="rpn_bbox_pred")
    ...
    rpn_cls_prob_reshape = mx.symbol.Reshape(data=rpn_cls_prob, shape=(0, 2 * num_anchor, -1, 0),
    ...
    rpn_roi = mx.symbol.contrib.Proposal(cls_prob=rpn_cls_prob_reshape, bbox_pred=rpn_bbox_pred, im_info=im_info,
                                         name='rpn_rois', feature_stride=16, scales=(4, 8, 16, 32), ratios=(0.5, 1, 2),
...
def resnet(units, num_stage, filter_list, num_class=2, \
           num_anchor=12, bottle_neck=True, bn_mom=0.9, \
    ...
    data = mx.sym.BatchNorm(data=data, fix_gamma=True, eps=2e-5, momentum=bn_mom, use_global_stats=bn_global,
                            name='bn_data')
    body = mx.sym.Convolution(data=data, num_filter=filter_list[0], kernel=(7, 7), stride=(2, 2), pad=(3, 3),
                              no_bias=True, name="conv0", workspace=workspace)
    body = mx.sym.BatchNorm(data=body, fix_gamma=False, eps=2e-5, momentum=bn_mom, use_global_stats=bn_global,
                            name='bn0')
...
def faster_rcnn50(num_class=2, bn_mom=0.99, bn_global=True):
    return resnet(units=[3, 4, 6, 3], num_stage=4, \
                  filter_list=[64, 256, 512, 1024, 2048], \
                  num_class=num_class, num_anchor=12, \
                  bottle_neck=True, bn_mom=bn_mom, \
                  bn_global=bn_global, workspace=512)
